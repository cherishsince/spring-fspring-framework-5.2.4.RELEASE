 ...
 * 该接口可以由单例bean实现，以便在常规的单例实例化算法之后执行一些初始化，
 * 避免意外的早期初始化带来的副作用（例如，来自{@link ListableBeanFactorygetBeansOfType}调用）。
 * 从这个意义上说，它是{@link InitializingBean}的替代方法，后者在bean的本地构造阶段结束时立即触发。
 *
