...
public abstract class AbstractHandlerMapping extends WebApplicationObjectSupport
	...
	protected void initApplicationContext() throws BeansException {
		...
		// <2> 扫描已注册的 MappedInterceptor 的 Bean 们，添加到 adaptedInterceptors 中
	...
	/**
	 * 初始化指定的拦截器，检查{@link MappedInterceptor MappedInterceptors}，
	 * 并在必要时调整{@link HandlerInterceptor}和 {@link WebRequestInterceptor HandlerInterceptor}和 {@link WebRequestInterceptor}。
	 * <p>
	...
	protected void initInterceptors() {
		...
		if (!this.interceptors.isEmpty()) {
			for (int i = 0; i < this.interceptors.size(); i++) {
				...
				// (adaptInterceptor(interceptor)：适配 HandlerInterceptor(也就是初始化))
	...
	public final HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
		// <1> 获得处理器。该方法是抽象方法，由子类实现(拿到的是 HandlerMethod)
	...
	protected HandlerExecutionChain getHandlerExecutionChain(Object handler, HttpServletRequest request) {
		// 转换/创建 HandlerExecutionChain 对象
